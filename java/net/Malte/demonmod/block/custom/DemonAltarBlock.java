package net.Malte.demonmod.block.custom;

import net.Malte.demonmod.world.dimension.DemonDimension;
import net.Malte.demonmod.world.dimension.DemonTeleporter;
...
public class DemonAltarBlock extends HorizontalBlock {

    public DemonAltarBlock(Properties builder) {
            super(builder);
        }
        ...
        // Wenn der Block aktiviert (angeklickt) wird, führt er die Aktion aus, den Spieler zu "teleportieren".
        @Override
        public ActionResultType onBlockActivated (BlockState state, World worldIn, BlockPos pos,
            ...
            if (!worldIn.isRemote()) {
                if (!player.isCrouching()) {
                    ...
                    if (server != null) {
                        if (worldIn.getDimensionKey() == DemonDimension.DDim) {
                            ServerWorld overWorld = server.getWorld(World.OVERWORLD);
                            if (overWorld != null) {
                                player.changeDimension(overWorld, new DemonTeleporter(pos, false));
                            }
                        } else {
                            ServerWorld kjDim = server.getWorld(DemonDimension.DDim);
                            if (kjDim != null) {
                                player.changeDimension(kjDim, new DemonTeleporter(pos, true));
                            }
                        }
